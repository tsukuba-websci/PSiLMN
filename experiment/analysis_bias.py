...
def created_figs(file_path: str, bias: str) -> None:
    ...
    network_type = file_path.split('/')[4]
    num_agents = file_path.split('/')[5].split('.')[0]
    ...
    for question in questions:
        ...
        for round_ in rounds:
            ...
            # Adjust color mapping based on correctness and centrality
            color_map = []
            for _, row in round_df.iterrows():
                if row['bias'] == "correct":  # Check if the node is a hub
                    color = 'darkgreen'
                elif row['bias'] == "incorrect":
                    color = 'darkred'
                else:
                    color = 'lightgreen' if row['correct'] else 'lightcoral'
            ...
            image_path = f'output/biased/{bias}/figs/{network_type}/{num_agents}_q{question}_r{round_num}.png'
        ...
        gif_path = f'output/biased/{bias}/figs/{network_type}/{num_agents}_q{question}.gif'
        gif_directory = os.path.dirname(gif_path)
        os.makedirs(gif_directory, exist_ok=True)  # Ensure the directory exists
...
if __name__ == "__main__":
    for bias in ["correct", "incorrect"]:
        ...
        output_analyse_path = Path(f"output/biased/{bias}/analysis/")
        output_analyse_path.mkdir(parents=True, exist_ok=True)
            
        # Specify the path to your CSV file
        csv_files = glob.glob(f'output/biased/{bias}/agent_responses/**/*.csv', recursive=True)
        ...
        with open(results_file_path, 'w', newline='') as results_file:
            ...
            for file in csv_files:
                ...
                # Create the figures for the current file
                created_figs(file, bias)
